...
class ButtonFieldInfo_Core extends TextFieldInfo {
  //------------------------------------------------
  ...
  _getIdListValue = (formFieldInfo, notSendGrid) => {
  ...
  getLocation = () => {
    return new Promise((resolve, reject) => {
      navigator.geolocation.getCurrentPosition(
        (info) => {
          resolve({
            location_Lat: info.coords.latitude,
            location_Long: info.coords.longitude,
          });
        },
        (error) => {
          reject(error);
  ...
  async _buttonAction(actionType, addExtraOpenFormParam) {
    ...
    if (this.data.getDataFromExternal) {
    ...
    switch (actionType || this.fieldInfo.button_ActionType) {
      ...
      case ButtonActionTypes.PrintByTemplate:
        ...
        if (
          Utils.getFileExtension(mrtFileNameForView) === undefined ||
          !Utils.getFileExtension(mrtFileNameForView).includes("mrt")
        ) {
          SystemClass.showErrorMsg(
      ...
      case ButtonActionTypes.DesignPrintTemplate:
        ...
        if (
          Utils.getFileExtension(mrtFileName) === undefined ||
          !Utils.getFileExtension(mrtFileName).includes("mrt")
        ) {
          SystemClass.showErrorMsg(
            "فایل گزارش صحیح نمی باشد. (پسوند فایل باید mrt باشد.)"
          );
      ...
      //PDF viewer in upload grid form
      case ButtonActionTypes.OpenPdfViewer:
        //if button is in grid then prop _grid point to grid component
        if (!this.fieldInfo._grid) {
          SystemClass.showErrorMsg("باید داخل Gird باشد!");
          return;
        }
        const pdfViewerFieldInfo = formFieldInfo
          .getFieldList()
          .find((f) => f.fieldType === FieldType.PdfViewer);
        if (!pdfViewerFieldInfo) {
          SystemClass.showErrorMsg("فیلد PdfViewer یافت نشد!");
          return;
        }

        pdfViewerFieldInfo.component.open(this.fieldInfo._row);
        break;
      ...
      case ButtonActionTypes.OpenDialog:
        ...
        if (enableIdList) {
          fieldValues = this._getIdListValue(formFieldInfo, true);
      ...
      case ButtonActionTypes.CallWebSvc:
        let otherInfo = {}
        if (this.fieldInfo['button_Pushe_SendDeviceInfo'] && UiSetting.GetSetting("sendPusheNothification") === true) {
          window.Pushe.init("3e4ll472qq9013od");
          window.Pushe.subscribe();
          await window.Pushe.getDeviceId()
            .then((deviceInfo) => {
              Object.assign(otherInfo, { deviceId: deviceInfo })
            })
            .catch((error) => {
              console.warn(error)
              SystemClass.showErrorMsg(
                'خطا در گرفتن اطلاعات دستگاه رخ داد.',
                71001,
              )
              return
            })
        ...
        //if data imported from external component such hac FINTRAC TRANSACTIONS
        if (this.data.getDataFromExternal === "true") {
          this.data.externalData = this.props.externalData;
          fieldValues = this.data.externalData;
        ...
        // get geoLocation
        if (this.fieldInfo["button_SendLocation"]) {
          let geoHasError = null;
          // TODO: Implement here! to get users current location.
          // SystemClass.showWarningMsgCustomDuration(
          //   "درحال دریافت موقعیت مکانی...",
          //   60000
          // );
          ...
          if (geoHasError) {
            if (
              ...
              SystemClass.showErrorMsg("لطفا GPS دستگاه را روشن کنید.", 71000);
              return;
            }
            SystemClass.showErrorMsg(
              "خطا هنگام دریافت موقعیت مکانی رخ داد.",
              71000
            );
          }
        }
        ...
        SystemClass.webService_UpdateForms(
          ...
          formModel,
          {
            ...
            paramList: Utils.mergeObject(
              ...
              buttonFormParams),
            fieldValues: Utils.mergeObject(fieldValues,
              otherInfo
            ),
          },
          { binary_1: thumbnail }
          ...
          .then((js) => {
            ...
            //for getting combo data from fintrac components
            if (this.props.onGetDatasource) {
              this.props.onGetDatasource();
            }
  ...
  _handleClick(event, actionType, addExtraOpenFormParam) {
    if (event) {
      //Because of event bubbling in Fintrac Transaction for Cleaning State
      if (!this.data.getDataFromExternal) {
...
export default ButtonFieldInfo_Core;
