...
class ConnectionStatus extends BaseComponent {
    constructor(props) {
        ...
        this._handleWindowUpdateOnlineStatus =
            this._handleWindowUpdateOnlineStatus.bind(this);
    }
    ...
    componentDidMount() {
        ...
        // this.data.node = ReactDOM.findDOMNode(this); // تغییر ۳: این خط حذف شد
        this.forceUpdate();
    }
    ...
    render() {
        ...
        // تغییر ۴: یک شناسه منحصر به فرد برای هدف‌گیری تولتیپ ایجاد شد
        const elementId = "connection-status-tooltip-target";

        return (
            <div
                // تغییر ۵: ref و id به عنصر اصلی متصل شدند
                ref={this.nodeRef}
                id={elementId}
                className={["ConnectionStatus"].filter((c) => c).join(" ")}
            >
                <Button className={"Menu__icon ConnectionStatus__button"} outline color="light">
                    <FontAwesome
                        className={[
                            "ConnectionStatus__icon",
                            offline && "ConnectionStatus__icon--offline",
                        ]
                            .filter((c) => c)
                            .join(" ")}
                        name="signal"
                ...
                {
                    // تغییر ۶: تولتیپ به جای node از id استفاده می‌کند
                    <UncontrolledTooltip target={elementId}>{status}</UncontrolledTooltip>
                }
            </div>
        );
    }
