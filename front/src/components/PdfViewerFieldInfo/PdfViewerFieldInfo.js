...
import PdfsViewer from "../ImageViewer/PdfsViewer";

// import Utils from '.././Utils.js';

class PdfViewerFieldInfo extends TextFieldInfo {
  initialize() {
    //for binding
    this.state = {
      srcSet: [],
  ...
  _initData(row) {
    ...
    const gridInfo = this.getFieldInfo(pdfViewer_Grid_FieldName);
    if (!gridInfo) {
  ...
  // fileName = this.fieldInfo._row[this.fieldInfo.dataSource_FileName_FieldName];

  // fileUrl = WebService.getFileUrl(
  //   this.fieldInfo._row[this.fieldInfo.dataSource_Link_FieldName]
  // );

  render() {
    //  return <PdfViewer src={this.state.srcSet[this.state.currImg]} />;
...
export default PdfViewerFieldInfo;
