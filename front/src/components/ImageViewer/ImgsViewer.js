...
class ImgsViewer extends Component {
          ...
          >
            <Fragment>
              <div
                ...
                style={{ marginBottom: offsetThumbnails - 50, maxWidth: width }}
              >
                {imgLoaded2 && this.renderHeader(theme)}
                {this.renderImgs()}
                {this.renderSpinner()}
                {imgLoaded2 && this.renderFooter(theme)}
              </div>
              {/* {imgLoaded2 && this.renderThumbnails(theme)} */}
              {imgLoaded2 && this.renderArrowPrev(theme)}
              {imgLoaded2 && this.renderArrowNext(theme)}
  ...
  renderImgs() {
    ...
    const fileExtention = img.row.fileExtension;
    ...
    return (
      <figure className={css(this.classes.figure) + "mt-4"}>
        {/* Re-implement when react warning 'unknown props'
        https://fb.me/react-unknown-props is resolved
        <Swipeable onSwipedLeft={this.gotonext} onSwipedRight={this.gotoPrev} /> */}
        {fileExtention.trim() === "pdf" && (
          <PdfViewer
            src={this.props.src}
            style={{
              cursor: onClickImg ? "pointer" : "auto",
              maxHeight: `calc(90vh - ${heightOffset} - 5rem)`,
            }}
            className={css(
              this.classes.img,
              imgLoaded && this.classes.imgLoaded
            )}
          />
        )}
