...
import "./Menu.css";
// BaseComponent دیگر مورد نیاز نیست چون از هوک‌ها استفاده می‌کنیم
...
// تبدیل کامپوننت به یک کامپوننت تابعی
function Menu() {
  ...
  // **مهم‌ترین تغییر: مدیریت مسیریابی با useNavigate**
  const _handleOnItemSelect = (menuItem) => {
    ...
    setOpenMenus({}); // بستن همه منوها پس از انتخاب
    ...
    if (menuItem.menuItem_Frame_UseFrame && menuItem.menuItem_Frame_Url) {
      ...
      navigate(`/form/${menuItem.menuItem_FormCid}`);
    ...
    return (
      ...
      >
        {!isSubMenu ? (
          ...
          >
            <div>
              <span className="MenuItem__displayName">{menuItem.menuItem_DisplayName}</span>
              {haveSubMenu && (
        ...
        {haveSubMenu && (
          <div tabIndex="-1" role="menu" aria-hidden="false" className={className}>
            {subMenuList.map((item, subIndex) => _elementGetMenuItem(item, subIndex, true))}
          </div>
        )}
      </div>
    );
    ...
    >
      ...
      <div className="Menu__container">
        ...
        <UncontrolledDropdown nav inNavbar>
          ...
          <DropdownMenu end>
            <DropdownItem onClick={_handleLogoutClick}>
            ...
            <DropdownItem onClick={_handleOnProfileClick}>
